...
export function handleClientProxyRequest(
  ...
  const sendHeadAndStream = async () => {
    try {
      dc.send(head);

      if (!hasBody) return;
      port.postMessage({ type: REQUEST_BODY_START });

      const requestStream = new ReadableStream<Uint8Array<ArrayBuffer>>({
        start(controller) {
          port.onmessage = (ev) => {
            switch (ev.data.type) {
